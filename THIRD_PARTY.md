...
The database is no longer installed with the game. SDL2's
...
are not used by the `v0.0.5` build. The UI font, menu graphics, miner forms,
and short audio effects are generated by original repository code.

Lua 5.1.5 was vendored through `v0.0.3` for the deterministic catalog sandbox
that fed the Miner's Index. The v0.0.5 build is ISO C only and retains neither the
interpreter and the screen it served, so the vendored tree, `LICENSES/Lua-5.1.txt`,
and the `.lua` data files are gone. No Lua code remains in the build or in any
shipped artifact.
