...
| Android phone host | Android SDK 34; NDK 26.3.11579264; pinned SDL2 2.30.12; ARMv7 and ARM64 APK | PLANNED | The source adapter packages the existing game data, runs the same SDL2 host, and adds TalkBack-labelled phone controls. CI is configured to compile the APK, but no Android device boot, display, audio, controller, or touch acceptance record exists yet. |
