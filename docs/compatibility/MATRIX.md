...
| SDL deterministic load | Current local checkout; clean strict SDL2 builds at `-O0` and `-O2` | RUNS | Both optimisation levels reproduced the repaired 600-tick stream including the interactive dropship start: fired 18, explosions 16, crushes 0, effects 796, awake 4294, canonical hash `3b60903a`. No timing threshold was applied. |
...
| Current-source test package | Explicitly allowed dirty local checkout; clean Release+NASM package build | RUNS | Package CTest completed 31/31. The temporary package produced binary and Corresponding Source archives, validated SPDX SBOMs and SHA-256 checksums, and a 560,087-byte staged payload. Under the active 10,485,760-byte ceiling that is 5.3% with 9,925,673 bytes headroom. Its `0.0.4-feedback-test` name makes it non-release evidence, not an artifact to publish. The package was made before this evidence-row wording refresh. |
...
| Playable payload budget | Staged Release tree | RUNS | 505,735 bytes, 4.8% of the active 10,485,760-byte (10 MiB) ceiling, 9,980,025 bytes of headroom. The budget covers the binary plus the staged `share/` tree; documentation, licences and evidence are tester-archive material and excluded. |
